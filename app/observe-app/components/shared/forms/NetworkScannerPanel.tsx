...
export const NetworkScannerPanel: FC<NetworkScannerPanelProps> = ({ onAdd }) => {
    ...
    const handleScan = async () => {
        ...
        setFoundHosts([]);
        try {
            const response = await fetch(`${API_URL}/network/scan`, { method: 'POST' });
            if (!response.ok) {
        ...
        <div className="bg-slate-800/50 p-6 rounded-xl border border-white/10">
            ...
            {isScanning && 
                <p className="text-center text-slate-400 my-4">
                    Scanning for active hosts on the local network. This may take a few minutes...
                </p>
            }
            ...
            {foundHosts.length > 0 && (
                <div className="space-y-2 max-h-96 overflow-y-auto pr-2">
                    <h4 className="font-semibold text-slate-300 mb-3">Found {foundHosts.length} Active Host(s):</h4>
                    {foundHosts.map(host => (
                        <div key={host} className="flex justify-between items-center bg-slate-900/50 p-3 rounded-lg animate-fade-in">

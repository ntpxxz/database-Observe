...
const InsightIcon: FC<{ type: string }> = ({ type }) => {
  switch (type) {
    case "slow_query":
      return <TrendingUp size={16} className="text-orange-400 mr-2 flex-shrink-0" />;
    case "long_running_query":
      return <Hourglass size={16} className="text-yellow-400 mr-2 flex-shrink-0" />;
    case "blocking_query":
    ...
    case "error":
      return <AlertCircle size={16} className="text-red-400 mr-2 flex-shrink-0" />;
    default:
      return <HelpCircle size={16} className="text-slate-500 mr-2 flex-shrink-0" />;
  }
};
...
export const PerformanceInsightsTable: FC<PerformanceInsightsTableProps> = ({ insights, serverName, isLoading = false }) => {
  ...
  if (!Array.isArray(insights) || insights.length === 0) {
    return <p className="text-slate-400 text-sm text-center py-4">No significant performance issues found. System looks healthy!</p>;
  }

  const totalPages = Math.ceil(insights.length / ROWS_PER_PAGE);
  const startIndex = (currentPage - 1) * ROWS_PER_PAGE;
  const currentInsights = insights.slice(startIndex, startIndex + ROWS_PER_PAGE);
      ...
      <table className="w-full text-sm text-left">
        <thead className="text-xs text-slate-500 uppercase">
          <tr>
            ...
            <th className="px-4 py-3 font-normal">Query / Insight Title</th>
            <th className="px-4 py-3 font-normal text-right">Avg. Duration (s)</th>
            <th className="px-4 py-3 font-normal text-right">Calls</th>
            <th className="px-4 py-3 font-normal text-center">Analyze</th>
          </tr>
        </thead>
        <tbody>
          {currentInsights.map(insight => {
            ...
            const durationDisplay = timeValueInMs !== null ? (timeValueInMs / 1000).toFixed(2) : "N/A";
            ...
            return (
              ...
              >
                ...
                <td className="px-4 py-3 font-semibold text-amber-300 text-right">{durationDisplay}</td>
                <td className="px-4 py-3 text-slate-400 text-right">{callCount ? callCount.toLocaleString() : "N/A"}</td>

export const parseNodeExporterMetrics = (
  metricsText: string,
): { cpu?: number; memory?: number } => {
  const lines = metricsText.split("\n");
  const metrics: { cpu?: number; memory?: number } = {};
  try {
    const memTotalLine = lines.find((line) =>
      line.startsWith("node_memory_MemTotal_bytes"),
    );
    const memFreeLine = lines.find((line) =>
      line.startsWith("node_memory_MemFree_bytes"),
    );
    const memBuffersLine = lines.find((line) =>
      line.startsWith("node_memory_Buffers_bytes"),
    );
    const memCachedLine = lines.find(
      (line) =>
        line.startsWith("node_memory_Cached_SReclaimable_bytes") ||
        line.startsWith("node_memory_Cached_bytes"),
    );
